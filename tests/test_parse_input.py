...
class Test_ParseInput(unittest.TestCase):
    ...
    def test_LoadScores(self):
        ...
        self.assertEqual(scores[0].GameFormat,'Podjazd')
        ...
        self.assertEqual(scores[1].GameFormat,'Zgrupowanie')
    ...
    def test_LoadAllScores(self):
        scores = parse_input.load_all_scores('tests')
    ...
    def test_LoadMasterData(self):
        myMDM = mdm.MasterDataDict()
        test_mdm_filename = "tests/mdmtest.csv"
        parse_input.load_master_data(myMDM, test_mdm_filename)
        self.assertEqual(myMDM.getGoldenKey('SampleDP','SampleNK'), 'SampleGK')
